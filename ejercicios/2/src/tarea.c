...
{
	return snprintf(buf, size, "la tarea %d esta enviada por %s se llama  %s y la descripcion  %s con prioridad %d",
			t->id, t->usuario,t->nombre_tarea, t->desc_tarea,t->prioridad);
}
